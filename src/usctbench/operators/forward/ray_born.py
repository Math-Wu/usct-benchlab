...
Heterogeneous phase uses fast marching; WKB amplitude solves the transport
identity grad(T).grad(log A) = -Laplacian(T)/2 on its causal stencil.
...
def transport_log_amplitude(tape, slowness: np.ndarray, spacing, source):
    """Causal first-order transport solve (no caustic phase corrections)."""
    shape = slowness.shape
    field = tape.times.reshape(shape)
    laplacian = sum(
        np.gradient(np.gradient(field, h, axis=axis), h, axis=axis)
    ...
    for node in tape.order:
        if tape.parents[node, 0] < 0:
            continue
        value = 0.0
        for parent, weight in zip(tape.parents[node], tape.weights[node]):
            if parent >= 0:
                value += weight * log_amplitude[parent]
        denominator = slowness.ravel()[node] / tape.local_derivative[node]
        log_amplitude[node] = value - 0.5 * laplacian[node] / denominator
...
class RayBornOperator:
    ...
    def _heterogeneous_fields(self):
        ...
        for index, source in enumerate(model.source_coordinates):
            ...
            self.eikonal_solves += 2
            delay = tape.times - reference.times
            log_ratio = transport_log_amplitude(
                tape, slowness, self.grid.spacing_m, source
            ) - transport_log_amplitude(reference, water, self.grid.spacing_m, source)
