...
NBP-D 的二次项 TRF (`outer_high_d_single_trf64_b5c27d5`) 已完成并选中第 20 步，
以 `max_iterations` 停止，求解耗时 4528.8 s。组织区 RMSE 21.1118 m/s、
PSNR 15.0906 dB、SSIM 0.27405，训练/留出相对残差为 0.0001120 / 0.100703。
相对上述 GN `peakqc_r3`，指标只有小幅改善，仍没有解决图像模糊。
TRF 终点的缩放最优性指标为 1.95e-5，高于设定的 1e-6；不能把预算停止称为
已经达到驻点。训练残差很低也不等于留出泛化、空间可辨识性或组织成像已通过。
...
该设置此前已在 HET 对照中固定；本例用于开发验证，不声称盲测或已改善成像。
...
两项均已启动，尚无最终成像结论；它们不是低到高频率 continuation，也不是独立频率盲测。
